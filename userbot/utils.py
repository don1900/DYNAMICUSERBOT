...
from userbot import CMD_LIST, LOAD_PLUG, LOGS, SUDO_LIST, DYNAMIC, DYNAMICBOT, bot
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        LOGS.info("ɖʏռǟʍɨƈɮօȶ ~ " + shortname)
    else:
        import userbot.utils
        ...
        mod.bot = DYNAMIC
        mod.borg = bot
        # mod.DYNAMICBOT = DYNAMIC
        mod.DYNAMICBOT = DYNAMICBOT
        mod.tbot = DYNAMICBOT
        mod.DYNAMIC = DYNAMIC
        mod.tgbot = bot.tgbot
        mod.Var = Var
        ...
        mod.delete_DYNAMIC = delete_DYNAMIC
        mod.eod = delete_DYNAMIC
        mod.admin_cmd = admin_cmd
        mod.DYNAMIC_cmd = admin_cmd
        mod.sudo_cmd = sudo_cmd
        # support for DYNAMICBOT originals
        sys.modules["DYNAMICBOT.utils"] = userbot.utils
        sys.modules["DYNAMICBOT"] = userbot
...
def load_addons(shortname):
    if shortname.startswith("__"):
        ...
        name = "userbot.plugins.DYNAMICBOT-Addons.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["DYNAMICBOT-Addons." + shortname] = mod
...
def load_abuse(shortname):
    if shortname.startswith("__"):
        ...
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        LOGS.info("DYNAMICBOT-Abuse ~ " + shortname)
    else:
        import importlib
        import sys
        from pathlib import Path
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                DYNAMICreg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                DYNAMICreg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(DYNAMICreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                DYNAMICreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
async def delete_DYNAMIC(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 5
    if event.sender_id in Config.SUDO_USERS:
        ...
        DYNAMICevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await DYNAMICevent.delete()
