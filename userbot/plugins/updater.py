...
BOT_IS_UP_TO_DATE = "**The DYNAMICBOT** is up-to-date sir."
NEW_BOT_UP_DATE_FOUND = (
    "new update found for {branch_name}\n"
    "changelog: \n\n{changelog}\n"
    "updating your DYNAMICBOT ..."
)
NEW_UP_DATE_FOUND = (
    "New update found for {branch_name}\n" "`updating your DYNAMICBOT...`"
)
...
DYNAMICbot_info = "https://raw.githubusercontent.com/don1900/DYNAMICUSERBOT/DYNAMICBot/DON-info.json"
...
async def DYNAMIC_info(DYNAMICbot_info):
    infos = requests.get(DYNAMICbot_info).json()
    _version = infos["DYNAMICBOT-INFO"]["version"]
    _release = infos["DYNAMICBOT-INFO"]["release-date"]
    _branch = infos["DYNAMICBOT-INFO"]["branch"]
    _author = infos["DYNAMICBOT-INFO"]["author"]
    _auturl = infos["DYNAMICBOT-INFO"]["author-url"]
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"🌚 Do `.update build` to update your **DYNAMICẞø†** !!"
        )
...
async def upstream(event):
    ...
    off_repo = "https://github.com/don1900/DYNAMICBOT"
    os.chdir("/app")
    git_DYNAMIC = f"rm -rf .git"
    try:
        await runcmd(git_DYNAMIC)
    except BaseException:
        pass
    txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
    try:
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    _version, _release, _branch, _author, _auturl = await DYNAMIC_info(DYNAMICbot_info)
    await event.edit(
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(
            f"**Your Lêɠêɳ̃dẞø† Is UpToDate**\n\n**Version :**  __{DYNAMICversion}__\n**Oɯɳҽɾ :**  {DYNAMIC_mention}"
        )
    else:
        await event.edit(
