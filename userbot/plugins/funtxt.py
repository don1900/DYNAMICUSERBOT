...
async def hmm(DYNAMIC):
    if DYNAMIC.fwd_from:
        return
    reactcat = nekos.textcat()
    await edit_or_reply(DYNAMIC, reactcat)
...
async def hmm(DYNAMIC):
    if DYNAMIC.fwd_from:
        return
    whyDYNAMIC = nekos.why()
    await edit_or_reply(DYNAMIC, whyDYNAMIC)
...
async def hmm(DYNAMIC):
    if DYNAMIC.fwd_from:
        return
    factDYNAMIC = nekos.fact()
    await edit_or_reply(DYNAMIC, factDYNAMIC)
