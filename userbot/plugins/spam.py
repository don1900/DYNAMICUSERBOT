...
async def bigspam(DYNAMIC):
    if not DYNAMIC.text[0].isalpha() and DYNAMIC.text[0] not in ("/", "#", "@", "!"):
        DYNAMIC_msg = DYNAMIC.text
        DYNAMICBOT_count = int(DYNAMIC_msg[9:13])
        DYNAMIC_spam = str(DYNAMIC.text[13:])
        for i in range(1, DYNAMICBOT_count):
            await DYNAMIC.respond(DYNAMIC_spam)
    ...
    if lg_id:
        if event.is_private:
            ...
            await event.client.send_message(
                lg_id,
                "#WSPAM\n"
                + f"Word Spam was executed successfully in {DYNAMIC_mention} chat with : `{message}`",
            )
...
CmdHelp("spam").add_command(
    ...
    "bigspam",
    "<count> <text>",
    "Sends the text 'X' number of times. This what DYNAMICBOT iz known for. The Best BigSpam Ever",
    ".bigspam 5000 hello",
).add_command(
    "cspam",
    "<sentence>",
    "Spam the chat with every letter in given sentence as new message",
    ".cspam DYNAMICBOT IS OP",
).add_command(
    "wspam",
    "<sentence>",
    "Spams the chat with every word in given sentence as new message",
    ".wspam DYNAMICBOT IS OP",
).add()
