...
async def variable(var):
    ...
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await eor(
            var,
            f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit DYNAMIC_grp for help.",
            link_preview=False,
        )
    ...
    if exe == "get":
        ...
        try:
            ...
            DYNAMIC = "**ConfigVars**:" f"\n\n {variable} = `{heroku_var[variable]}`\n"
            if "DYNAMIC_STRING" in variable:
                await eor(
                    var, "DYNAMIC String is a Sensetive Data.\nProtected By DYNAMICBOT"
                )
                return
            elif variable in heroku_var:
                await eor(var, DYNAMIC)
            else:
...
async def _(event):
    ...
    DYNAMIC_data = app.get_log()
    await eor(event, DYNAMIC_data)
