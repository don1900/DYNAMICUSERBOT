...
async def _(event):
    ...
    if not (_reply and (_reply.media)):
        ...
        return
    DYNAMIC = await _reply.download_media()
    if DYNAMIC.endswith((".tgs")):
        legen_ = await eor(event, "**Memifying 🌚🌝**")
        cmd = ["lottie_convert.py", DYNAMIC, "pic.png"]
        file = "pic.png"
        ...
        stderr.decode().strip()
        stdout.decode().strip()
    elif DYNAMIC.endswith((".webp", ".png")):
        legen_ = await eor(event, "**Memifying 🌚🌝**")
        pic = Image.open(DYNAMIC)
        pic.save("pic.png", format="PNG", optimize=True)
        file = "pic.png"
    else:
        await eor(event, "**Memifying 🌚🌝**")
        img = cv2.VideoCapture(DYNAMIC)
        tal, semx = img.read()
...
async def nope(event):
    DYNAMIC = event.pattern_match.group(1)
    if not DYNAMIC:
        if event.is_reply:
    ...
    troll = await bot.inline_query("DogeStickerBot", f"{(deEmojify(DYNAMIC))}")
    if troll:
        await event.delete()
...
async def nope(kraken):
    DYNAMIC = kraken.pattern_match.group(1)
    if not DYNAMIC:
        if kraken.is_reply:
    ...
    troll = await bot.inline_query("GooglaxBot", f"{(deEmojify(DYNAMIC))}")
    if troll:
        await kraken.delete()
        legen_ = await troll[0].click(Config.LOGGER_ID)
        if legen_:
...
async def nope(kraken):
    ...
    troll = await bot.inline_query("honka_says_bot", f"{(deEmojify(DYNAMIC))}.")
    if troll:
        await kraken.delete()
        legen_ = await troll[0].click(Config.LOGGER_ID)
        if legen_:
