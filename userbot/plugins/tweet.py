...
async def nope(DYNAMIC):
    ...
    if not DYNAMIC:
        if DYNAMIC.is_reply:
            ...
            await edit_or_reply(DYNAMIC, "I need some text to make a tweet🚶")
            return
    tweeter = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(DYNAMIC))}")
