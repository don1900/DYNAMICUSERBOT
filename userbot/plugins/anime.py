...
async def formatJSON(outData):
    ...
    if "errors" in res:
        ...
        # https://t.me/DYNAMIC_Userbot/19496
...
async def nope(lege):
    DYNAMIC = lege.pattern_match.group(1)
    if not DYNAMIC:
        if lege.is_reply:
    ...
    troll = await bot.inline_query("AniFluidbot", f".anime {(deEmojify(DYNAMIC))}")
...
async def nope(lege_):
    DYNAMIC = lege_.pattern_match.group(1)
    if not DYNAMIC:
        if lege_.is_reply:
            (await lege_.get_reply_message()).message
        else:
    ...
    troll = await bot.inline_query("AniFluidbot", f".manga {(deEmojify(DYNAMIC))}")
...
async def nope(lege_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".character {(deEmojify(DYNAMIC))}")
