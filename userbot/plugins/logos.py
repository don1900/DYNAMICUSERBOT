...
async def lg1(DYNAMICevent):
    event = await edit_or_reply(DYNAMICevent, "`Processing.....`")
    fnt = await get_font_file(event.client, "@DYNAMICFonts")
    if DYNAMICevent.reply_to_msg_id:
        rply = await DYNAMICevent.get_reply_message()
        logo_ = await rply.download_media()
    else:
        async for i in bot.iter_messages(
            "@DYNAMIC_MR_LOGOS", filter=InputMessagesFilterPhotos
        ):
            PICS_STR.append(i)
        pic = random.choice(PICS_STR)
        logo_ = await pic.download_media()
    text = DYNAMICevent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
    ...
    file_name = "DYNAMICBOT.png"
    img.save(file_name, "png")
    await bot.send_file(
        DYNAMICevent.chat_id,
        file_name,
