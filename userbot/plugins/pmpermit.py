...
DYNAMIC_ZERO = "Go get some sleep retard. \n\n**Blocked !!**"
DYNAMIC_FIRST = (
    "**🔥 DYNAMICBo† Prîvã†é Sêçürïty Prø†öçõl 🔥**\n\nThis is to inform you that "
    "{} is currently unavailable.\nThis is an automated message.\n\n"
    "{}\n\n**{}Please Choose Why You Are Here!!**".format(
        DYNAMIC_mention, CSTM_PMP, max_flood
    )
)
...
"""
    ...
    async def do_pm_permit_action(chat_id, event):
        ...
        if PM_WARNS[chat_id] == Config.MAX_FLOOD_IN_PM:
            r = await event.reply(DYNAMIC_ZERO)
            await asyncio.sleep(3)
        ...
        DYNAMIC_ = await tap[0].click(event.chat_id)
        PM_WARNS[chat_id] += 1
        chat_id = chat_id
        if chat_id in PREV_REPLY_MESSAGE:
            await PREV_REPLY_MESSAGE[chat_id].delete()
        PREV_REPLY_MESSAGE[chat_id] = DYNAMIC_
...
async def hehehe(event):
    ...
    if event.is_private:
        if not pm_sql.is_approved(chat.id):
            pm_sql.approve(
                chat.id,
                f"**My Boss iz here..{DYNAMIC_mention}'s Its Ur Lucky day Nibba😏!!**",
            )
...
CmdHelp("pm_permit").add_command(
    ...
    "listapproved", None, "Sends the list of all users approved by DYNAMICBot"
).add_info(
    "PM SECURITY"
).add_warning(
    "✅ Harmless Module."
