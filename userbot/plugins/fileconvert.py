...
from DYNAMICBOT.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(DYNAMIC):
    if DYNAMIC.fwd_from:
        return
    reply_to_id = DYNAMIC.message.id
    if DYNAMIC.reply_to_msg_id:
        reply_to_id = DYNAMIC.reply_to_msg_id
    event = await edit_or_reply(DYNAMIC, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await DYNAMIC.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await DYNAMIC.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    DYNAMICreply = await event.get_reply_message()
    if not DYNAMICreply or not DYNAMICreply.media or not DYNAMICreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if DYNAMICreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    DYNAMICevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, DYNAMICreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await DYNAMICevent.edit("`This file is not supported`")
            DYNAMICresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            DYNAMICfile = Path(
                await event.client.download_media(DYNAMICresponse, "./temp/")
            )
            DYNAMICgif = Path(await unzip(DYNAMICfile))
            DYNAMIC = await event.client.send_file(
                event.chat_id,
                DYNAMICgif,
            ...
            await event.client(
                functions.messages.SaveGifRequest(
                    id=types.InputDocument(
                        id=DYNAMIC.media.document.id,
                        access_hash=DYNAMIC.media.document.access_hash,
                        file_reference=DYNAMIC.media.document.file_reference,
                    ),
                    unsave=True,
                )
            )
            await DYNAMICevent.delete()
            for files in (DYNAMICgif, DYNAMICfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await DYNAMICevent.edit("Unblock @tgstogifbot")
            return
