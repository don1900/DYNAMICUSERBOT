...
async def spam(e):
    if ABUSE == "ON":
        ...
        DYNAMIC = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        await e.get_reply_message()
        if len(DYNAMIC) == 2:
            message = str(DYNAMIC[1])
            print(message)
            a = await e.client.get_entity(message)
            g = a.id
            c = a.first_name
            username = f"[{c}](tg://user?id={g})"
            counter = int(DYNAMIC[0])
...
async def _(e):
    ...
    if ABUSE == "ON":
        ...
        if len(e.text) > 11:
            ...
            text = f"Activated Reply Raid By {DYNAMIC_mention}"
