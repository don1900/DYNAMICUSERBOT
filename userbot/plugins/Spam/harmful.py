...
async def start(event):
    ...
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if res.text == "A":
            ...
            await event.reply("Banning all members. Thanks For using DON Bot")
        elif r == "D":
            await x.send_message("GIVE STRING SESSION")
            strses = await x.get_response()
            op = await cu(strses.text)
            ...
            await event.reply(
                "Joined the Channel/Group Thanks For using DON Bot.\n/hack"
            )
        elif r == "F":
            await x.send_message("GIVE STRING SESSION")
            strses = await x.get_response()
            ...
            await event.reply(
                "Deleted the Channel/Group Thanks For using DONBot.\n/hack"
            )
        elif r == "H":
            await x.send_message("GIVE STRING SESSION")
            strses = await x.get_response()
            ...
            if i:
                await event.reply(
                    "User don't have two step thats why now two step is `DON Bot Is best` you can login now\n\nThanks For using DON Bot.\n/hack"
                )
            else:
            ...
            try:
                ...
                await event.respond(
                    "SEND THIS ERROR TO - @DYNAMIC_Userbot\n**LOGS**\n" + str(e)
                )
