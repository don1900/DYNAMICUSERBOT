...
class DYNAMICGloballist(BASE):
    __tablename__ = "DYNAMICglobal_list"
    ...
    def __repr__(self):
        return "<DYNAMIC global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, DYNAMICGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


DYNAMICGloballist.__table__.create(checkfirst=True)

DYNAMICGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with DYNAMICGLOBALLIST_INSERTION_LOCK:
        broadcast_group = DYNAMICGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with DYNAMICGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(DYNAMICGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with DYNAMICGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(DYNAMICGloballist.keywoard)
            .filter(DYNAMICGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(DYNAMICGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(DYNAMICGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(DYNAMICGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(DYNAMICGloballist).all()
        for x in all_groups:
