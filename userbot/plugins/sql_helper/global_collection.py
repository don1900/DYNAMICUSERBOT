...
class DYNAMIC_GlobalCollection(BASE):
    __tablename__ = "DYNAMIC_globalcollection"
    ...
    def __repr__(self):
        return "<DYNAMIC Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, DYNAMIC_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


DYNAMIC_GlobalCollection.__table__.create(checkfirst=True)

DYNAMIC_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with DYNAMIC_GLOBALCOLLECTION:
        keyword_items = DYNAMIC_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with DYNAMIC_GLOBALCOLLECTION:
        keyword_items = SESSION.query(DYNAMIC_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with DYNAMIC_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(DYNAMIC_GlobalCollection.keywoard)
            .filter(DYNAMIC_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(DYNAMIC_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(DYNAMIC_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(DYNAMIC_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(DYNAMIC_GlobalCollection).all()
        for x in all_groups:
