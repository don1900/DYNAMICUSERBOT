...
class DYNAMIC_GlobalCollection_Json(BASE):
    __tablename__ = "DYNAMIC_globalcollectionjson"
...
DYNAMIC_GlobalCollection_Json.__table__.create(checkfirst=True)


def get_collection(keywoard):
    try:
        return SESSION.query(DYNAMIC_GlobalCollection_Json).get(keywoard)
    finally:
        SESSION.close()
