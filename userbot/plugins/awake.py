...
from userbot import DYNAMICversion, StartTime
...
async def amireallyalive(event):
    ...
    if DYNAMIC_IMG:
        DYNAMIC_caption = f"**{DYNAMIC_mention}**\n"

        DYNAMIC_caption += f"~~~~~~~~~~~~~~~~~~~~~~~\n"
        DYNAMIC_caption += f"     ⚜ 𝓛𝓮𝓰𝓮𝓷𝓭𝓑𝓸𝓽 𝓘𝓼 𝓐𝔀𝓪𝓴𝓮 ⚜\n"
        DYNAMIC_caption += f"•🔥• Lêɠêɳ̃dẞø†     : ν3.0\n"
        DYNAMIC_caption += f"•🔥• 𝚃𝙴𝙻𝙴𝚃𝙷𝙾𝙽      : `{version.__version__}`\n"
        DYNAMIC_caption += f"•🔥• 𝚄𝙿𝚃𝙸𝙼𝙴         : `{uptime}`\n"
        DYNAMIC_caption += f"•🔥• 𝙲𝙷𝙰𝙽𝙽𝙴𝙻        : [𝕮нαииєℓ](t.me/Official_DYNAMICBOT)\n"
        DYNAMIC_caption += f"•🔥• ᴹʸ 𝙶𝚁𝙾𝚄𝙿 : {CUSTOM_YOUR_GROUP}\n"

        await event.client.send_file(
            event.chat_id, DYNAMIC_IMG, caption=DYNAMIC_caption, reply_to=reply_to_id
        )
        await event.delete()
    else:
        await edit_or_reply(
