...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of DYNAMICBOT`")
        return
    DYNAMICBOT = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await DYNAMICBOT.edit("Committing to Github....")
        await git_commit(downloaded_file_name, DYNAMICBOT)


async def git_commit(file_name, DYNAMICBOT):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await DYNAMICBOT.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await DYNAMICBOT.edit("Cannot Upload Plugin")
    else:
        return await DYNAMICBOT.edit("`Committed Suicide`")
