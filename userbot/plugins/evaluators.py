...
async def _(event):
    ...
    DYNAMICuser = await event.client.get_me()
    if DYNAMICuser.username:
        curruser = DYNAMICuser.username
    else:
        curruser = "@Official_DYNAMICBOT"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    if EVAL == "ON":
        ...
        DYNAMICevent = await eor(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
    ...
    "exec",
    "<command>",
    "Execute a Terminal command on DYNAMICBOT server and shows details",
).add_command(
