...
from DYNAMICBOT import ALIVE_NAME, CmdHelp
...
DYNAMIC = DYNAMICBOT.uid

DYNAMIC = f"[{DEFAULTUSER}](tg://user?id={DYNAMIC})"


@DYNAMICBOT.on(admin_cmd(pattern="hvd$"))
@DYNAMICBOT.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@DYNAMICBOT.on(admin_cmd(pattern="vday$"))
@DYNAMICBOT.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
