...
<b>⚡ <a href='https://t.me/DYNAMIC_Userbot'>From DataBase of DYNAMICBOT</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
async def info(event):
    if event.fwd_from:
        return
    DYNAMIC = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await DYNAMIC.edit(caption, parse_mode="html")
    except Exception as e:
        print("Exception:", e)
        await eod(DYNAMIC, "`An unexpected error has occurred.`")
    return
