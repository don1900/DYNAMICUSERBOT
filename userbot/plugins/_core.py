...
async def install(event):
    ...
    b = 1
    owo = event.text[9:]
    DYNAMIC = await eor(event, "__Installing.__")
        ...
        try:
            ...
            try:
                if EVAL == "ON":
                    ...
                    return await eod(
                        DYNAMIC, "First Turn ON Eval CMD = `.set var EVAL ON`"
                    )
            except Exception as e:
                await eod(DYNAMIC, f"{e}")
                return os.remove(download_file_name)
        except Exception as e:
            await eod(DYNAMIC, f"**Failed to Install** \n`Error`\n{str(e)}")
...
CmdHelp("core").add_command(
    ...
    "cmds", None, "Gives out the list of modules in DYNAMICBOT."
).add_warning(
