...
async def fs(event):
    ...
    if not await is_admin(event, DYNAMICbooy):
    ...
    if args in ("off", "no", "disable"):
        ...
        rip = await check_him(channel, DYNAMICbooy)
        if rip is False:
            return await event.reply(
    ...
    if chat_db:
        try:
            ...
            if rip is False:
                ...
                DON = Config.BOT_USERNAME
                await bot(
                    EditBannedRequest(event.chat_id, event.sender_id, MUTE_RIGHTS)
                )
                response = await bot.inline_query(DON, "pm_warn")
...
@bot.on(events.InlineQuery(pattern="DYNAMICbooy"))
async def NOOBBOY(event):
    global rk
    event.builder
    DYNAMICproboy = [[Button.inline("Unmute Me 😊", data="DYNAMIC")]]
    NOOBBOYOP = DON.article(
        title="FORCE SUBSCRIBE", text="fsub", buttons=DYNAMICproboy
    )
...
@bot.on(events.callbackquery.CallbackQuery(re.compile(b"DYNAMIC")))
