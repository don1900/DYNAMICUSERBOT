...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        DYNAMIC = await event.client.get_entity(event.sender_id)
        if DYNAMIC.bot or DYNAMIC.verified or DYNAMIC.support:
            return

        DYNAMICm = f"[{get_display_name(DYNAMIC)}](tg://user?id={DYNAMIC.id})"
