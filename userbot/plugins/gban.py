...
The_DON = str(ALIVE_NAME) if ALIVE_NAME else "DYNAMIC User"
papa = borg.uid
...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        DYNAMICBOT = await edit_or_reply(ids, "Trying to gban this retard!")
    else:
        DYNAMICBOT = await edit_or_reply(ids, "`Ok! Gban ho rha h intzarrr karo....`")
    hum = await userbot.client.get_me()
    await DYNAMICBOT.edit(
    ...
    try:
        ...
        return await DYNAMICBOT.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        if user.id == 2082798662:
            return await DYNAMICBOT.edit(f"`How dare u trying to Gban my master`")
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await DYNAMICBOT.edit(f"Gbaning ho rha h 😂\n\nTotal Chats :- `{a}`")
            except:
                b += 1
    else:
        await DYNAMICBOT.edit(f"`Either reply to a user or gib me user id/name`")
    try:
        if gmute(user.id) is False:
            return await DYNAMICBOT.edit(
...
async def gunban(userbot):
    ...
    hum = await ids.client.get_me()
    if not sender.id == hum.id:
        DYNAMICBOT = await edit_or_reply(ids, "`Trying to ungban this kid...`")
    else:
        DYNAMICBOT = await edit_or_reply(ids, "`Ungban in progress...`")
    hum = await userbot.client.get_me()
    await DYNAMICBOT.edit(f"`Trying to ungban this kiddo...`")
    my_mention = "[{}](tg://user?id={})".format(hum.first_name, hum.id)
    f"@{hum.username}" if hum.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    try:
        ...
        return await DYNAMICBOT.edit("**Som3ting W3nt Wr0ng**")
    if user:
        if user.id == 2082798662:
            return await DYNAMICBOT.edit(
        ...
        await DYNAMICBOT.edit("**Reply to a user**")
    try:
        if ungmute(user.id) is False:
            return await DYNAMICBOT.edit("**Error! User already ungbanned.**")
    except:
        pass
    return await DYNAMICBOT.edit(
...
@borg.on(ChatAction)
async def handler(DYNAMIC):
    if DYNAMIC.user_joined or DYNAMIC.user_added:
        try:
            from userbot.plugins.sql_helper.gmute_sql import is_gmuted

            guser = await DYNAMIC.get_user()
        ...
        if gmuted:
            for i in gmuted:
                if i.sender == str(guser.id):
                    chat = await DYNAMIC.get_chat()
                    admin = chat.admin_rights
                    creator = chat.creator
                    if admin or creator:
                        try:
                            await client.edit_permissions(
                                DYNAMIC.chat_id, guser.id, view_messages=False
                            )
                            await DYNAMIC.reply(
