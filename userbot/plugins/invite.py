...
from DYNAMICBOT.utils import admin_cmd, edit_or_reply, eor, sudo_cmd
...
async def get_users(event):
    ...
    DYNAMIC_chat = legen_.lower
    restricted = ["@DYNAMIC_Userbot", "@Official_DYNAMICBOT"]
    DYNAMIC = await eor(event, f"**Inviting members from** {legen_}")
    if DYNAMIC_chat in restricted:
    ...
    if not sender.id == me.id:
        ...
        DYNAMIC = await edit_or_reply(event, "`processing...`")
    DYNAMIC = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await DYNAMIC.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await DYNAMIC.edit(
        "**⚜️[Ͳєямιиαℓ Տτατυѕ](https://t.me/DYNAMIC_Userbot)**\n\n`🔸Inviting Users.......`"
    )
    async for user in event.client.iter_participants(DYNAMIC.full_chat.id):
        try:
...
async def get_users(event):
    ...
    await DYNAMIC.edit("**TerminalStatus**\n\n`Collecting Users.......`")
