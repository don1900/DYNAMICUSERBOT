...
async def nope(aura):
    DYNAMIC = aura.pattern_match.group(1)
    if not DYNAMIC:
        if aura.is_reply:
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(DYNAMIC))}")
...
async def _(event):
    ...
    thumb_name = f"thumb{The_DON}.jpg"
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [DYNAMICBOT](https://t.me/DYNAMICSupport)",
            )
            await event.delete()
        except YouBlockedUserError:
