...
lnbot = TelegramClient("DON", API_ID, API_HASH).start(bot_token=token)


class DON(TelegramClient):
...
if Config.DYNAMIC_STRING:
    session_name = str(Config.DYNAMIC_STRING)
    sweetie = DON(session_name, Config.APP_ID, Config.API_HASH)
else:
